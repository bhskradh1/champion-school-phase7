...
export default async function AnnouncementsPage(){
  ...
  const {data:announcements}=await supabase.from('announcements').select('id,title,body,target_audience,is_published,published_at,expires_at,created_at,created_by,profiles!announcements_created_by_fkey(full_name)').order('created_at',{ascending:false}).limit(100);
