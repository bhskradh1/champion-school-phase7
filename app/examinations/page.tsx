...
export default async function ExaminationsPage(){
 ...
 const [{data:years},{data:classes},{data:sections},{data:subjects},{data:exams}]=await Promise.all([
  supabase.from('academic_years').select('id,name,is_current').order('starts_on',{ascending:false}),supabase.from('classes').select('id,name,grade,academic_year_id').order('grade'),supabase.from('sections').select('id,name,class_id').order('name'),supabase.from('subjects').select('id,name,code').order('name'),supabase.from('examinations').select('id,academic_year_id,term,name,starts_on,ends_on,marks_deadline,status,created_at').order('starts_on',{ascending:false,nullsFirst:false})]);
