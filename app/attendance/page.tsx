...
export default async function AttendancePage(){
  ...
  for(const s of scopes){
    const {data:ens}=await supabase.from('student_enrollments').select('student_id,admission_no,roll_no,profiles!student_enrollments_student_id_fkey(id,full_name)').eq('class_id',s.class_id).eq('section_id',s.section_id).eq('is_active',true);
    studentsByScope[`${s.class_id}:${s.section_id}`]=(ens||[]).map(e=>({id:e.student_id,full_name:(e.profiles as any)?.full_name||'Student',admission_no:e.admission_no,roll_no:e.roll_no})).sort((a,b)=>a.full_name.localeCompare(b.full_name));
  }
  const initialRecords:Record<string,any[]>={};
  for(const s of scopes){
    const {data:rs}=await supabase.from('attendance_records').select('student_id,status,note,attendance_date').eq('class_id',s.class_id).eq('section_id',s.section_id).order('attendance_date',{ascending:false}).limit(120);
    for(const r of rs||[]){const k=`${s.class_id}:${s.section_id}:${r.attendance_date}`;(initialRecords[k] ||= []).push({student_id:r.student_id,status:r.status,note:r.note});}
  }
